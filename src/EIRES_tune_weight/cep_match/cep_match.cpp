...
public:
	...
	bool processEvent()
	{
		...
		//event.attributes[Query::DA_EXTERNAL_FLAG] = 1;
...
int main(int _argc, char* _argv[])
{
    ...
    string partialMatchOutPutFilePrefix = HOMEDIR+"/CEP_load_shedding/src_PM_Distribution_test/NormalEventStreamGen/SampleData/PM";
    string suffix = "none";
	...
	int c;
	while ((c = _free_getopt(_argc, _argv, "F:c:q:p:m:n:r:D:T:f:C:Z:W:w:L:u:tsIPRGMVzhaoOAB")) != -1)
	{
		switch (c)
		{
        case 'F':
        ...
        case 'Z':
            ...
            break;
        case 'W':
            PatternMatcher::Transition::consumptionPolicy = stoi(string(_free_optarg));
            break;
        case 'w':
            UuFuWeight = stod(string(_free_optarg));
            break;
        case 'L':
    ...
    unordered_map<uint64_t, uint64_t> FutureUtilityMap;

    cache.setUtilityMap(&UtilityMap);
    cache.setFutureUtilityMap(&FutureUtilityMap);
    cache.setUuFuWeight(UuFuWeight);
    ...
    prog.readEventStreamFromFiles(streamFile, eventCnt, false); // don't change the distribution of event C
    ...
    for(auto && s : prog.m_Matcher.m_States)
    {
        ...
        s.setFutureUtilityMap(&FutureUtilityMap);
        s.setUuFuWeight(UuFuWeight);
    }
    ...
    //prog.m_Matcher.m_States[2].setIndexAttribute(1);
    ...
    //_condition.param[2] = 1;  // d.v1 attribute index.
    //_condition.constant = 0;
    //_condition.op = PatternMatcher::OP_ADD;
    //_condition.op2 = PatternMatcher::OP_GREATER;
    //_condition.op2 = PatternMatcher::OP_EQUAL;
    //_condition.op2 = PatternMatcher::OP_LESSEQUAL;
...
//     t.updateHandler(prog.m_Matcher.m_States[t.from], prog.m_Matcher.m_States[t.to]);
   ...
   // for(auto && t : prog.m_Matcher.m_Transitions)
   //     if(t.from == 3 && t.to == 4)
   //     {
   //         t.conditions.push_back(_condition);
   //         t.updateHandler(prog.m_Matcher.m_States[t.from], prog.m_Matcher.m_States[t.to]);
