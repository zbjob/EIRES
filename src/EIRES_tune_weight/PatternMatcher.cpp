    ...
    for( auto&& timeslice:buffers){
        ...
        for (auto&& attr: timeslice){
            ...
            while (!attr.front().empty() && attr.front().front() < _value) // the first deque stores timestamps
            {
                ...
                if(m_externalIdx)
                {
                    attr_t key = attr[m_externalIdx].front();
                    if(attr.front().front() != lastTimestamp)
                    {
                        lastTimestamp = attr.front().front();
                        (*m_lastTimeWindowFutureUtility)[key] = (*m_KeyUtility)[key]; 
                    }
...
void PatternMatcher::Transition::executeTransition(State & _from, State & _to, int timeslice, int cluster, uint32_t _idx, const attr_t* _attributes)
{
    ...
    if(_to.m_externalIdx) 
    {
        ...
        attr_t util = _to.UuFuWeight*(*_to.m_KeyUtility)[key] + (1-_to.UuFuWeight)*(*_to.m_lastTimeWindowFutureUtility)[key];

        //prefetching
        if(FLAG_prefetch && !_to.m_externalComIdx && !_to.m_externalFlagIdx) 
        {
        ...
        if(!FLAG_delay_fetch && _to.m_externalComIdx && attributes[_to.m_externalFlagIdx]) 
        {
            ...
            if(payload) //cache hit
            {
                ...
                _to.m_Cache->update(key, payload, util, 0);
            }
            else
            {
                ...
                payload = key;
                _to.m_Cache->update(key, payload, util, 3);
        ...
        if(FLAG_delay_fetch && _to.m_externalComIdx && attributes[_to.m_externalFlagIdx]) 
        {
            ...
            else
            {
                ...
                //set go-through probability to balance the bloking fetch operation and PM blow-ups
                if(_to.type == ST_ACCEPT || Dice_roll <= fetchProbability) 
                {
                    int latency = fetch_latency_distribution(_m_generator) * fetch_latency; 
                    this_thread::sleep_for(chrono::microseconds(latency));
