...
struct Query
{
	...
	struct AttrOperation {
		int operation;
		std::pair<uint32_t,uint32_t> rangeOp;
	};
	...
	std::vector<AttrOperation> returnOp; // operation event and attr for select


	std::vector<Aggregation> aggregation;
	map<uint32_t,Expression> exp;
...
public:
	...
	struct Callbacks
	{
		std::function<void(uint32_t, const attr_e*)>	insertEvent[PatternMatcher::ST_MAX];
		std::function<void(uint32_t, const attr_e*)>	timeoutEvent;
	};
